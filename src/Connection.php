...
//        $this->transactionIsolation = $this->getPlatform()->getDefaultTransactionIsolation();
    ...
    public function format(QueryBuilder $table):QueryBuilder
    {
        ...
        foreach ($this->parts as $part) {
            ...
            if ($condition == 'where' && (count($part[1]) > 1)) {//兼容处理
                if (count($part[1]) == 2) {
                    if (is_array($part[1][1])) {
                        $value = $part[1][0] . ' IN (' . join(',', $part[1][1]) . ')';
                    } else {
    ...
    public function multiInsert(array $update)
    {
        $table = $this->createQueryBuilder()->multiInsert($this->table);
        $table = $this->format($table);
        foreach ($update as $k => $v) {
            foreach ($v as &$vv) {
                if (!is_numeric($vv)) {
    ...
    {
        $table = $this->createQueryBuilder()->delete($this->table);
        $table = $this->format($table);
        if (empty($table->getPart('where'))) {
            throw new Exception('where is must when delete opt!! ');
        }
        return $table->execute();
    }
