...
class QueryBuilder
{
    ...
    public function multiInsert($insert)
    {
        $this->type = self::MULTI_INSERT;
    ...
    public function getQuery()
    {
        ...
        switch ($this->type) {
            ...
            case self::MULTI_INSERT:
                $query = $this->getMultiInsertQuery();
                break;
            case self::UPDATE:
    ...
    private function getMultiInsertQuery()
    {
        $values = '';
        foreach ($this->parts['set'] as $v) {
            $values .= ' (' . implode(', ', $v) . '),';
        }
        $values = trim($values, ',');
        return 'INSERT INTO ' . $this->parts['from'][0]['table'] .
        ' (' . implode(', ', array_keys($this->parts['set'][0])) . ')' .
        ' VALUES' . $values;
    }
